<?php session_start();
if(empty($_SESSION['username'])){header("Location:loginAdmin.php?type=addmarksReTest"); exit;}
$u=$_SESSION['username'];
//echo "user= $u";
?>
<html>
<head>
    <title>Add Marks ReTest</title>
...
//'$ID', '$searchTestDate', '$searchSubject','$searchExamTitle','$searchTotMarks', this.value
    function InsertData(Result_Id, ID, searchClass, searchTestDate, searchSubject, searchExamTitle, searchTotMarks, searchMarksObtained)
	{
		...
		var url="InsertMarksInDB_ReTest.php";
...
<form action="AddMarks_ReTest.php" method="get" id="formControler">
    <tr>
        <td>
        ...
        <td>
            <label for="TestDate" class="startDateDiv">Re-Test Date: </label>
        </td>
        <td>
            <input
            type="date"
...
<td>
		...
		<td>
		    	&nbsp;&nbsp;<button type="submit" class="btn" style="color:blue;"><a  style="text-decoration:none;" href="https://pa3tuitionpoint.000webhostapp.com/DipsMarks_ReTest.php" target="_Blank">ReTest Marks</a></button>
...
//echo '<script>alert("Message")</script>';
   ...
   $searchTotMarks = $_GET['TotMarks'];
    ...
    echo "<tr> <th>Result ID</th> <th>Name</th><th>Marks<br>Obtained</th><th>Total<br>Marks</th><th>Percentage(%)</th>        </tr>";





    $sql = "SELECT * FROM `StInfo` WHERE Class='$searchClass' AND Status='Active'";
	//echo "sql: $sql";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
			...
			$sDate = date("jMy", strtotime($searchTestDate));

			$Result_Id=$ID."_$searchSubject"."_$sDate";


			$sql333    = "SELECT TotMarks, MarksObtained, Percentage FROM StResult_ReTest where Result_Id='$Result_Id'";
           // echo "<br> sql333= $sql333";    
			...
			echo "<td title='Already Filled'>$perc %</td>";
			}
			else
			{			
