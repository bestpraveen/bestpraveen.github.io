...
<head>
	...
	function SearchAdvance(searchClass, searchSubject, searchYear)
	  {		
		window.location.href ='DipsMarks.php' + '?SearchAdvance=Yes' + '&searchClass=' + searchClass + '&searchSubject=' + searchSubject + '&searchYear=' + searchYear;
      }
...
if ($searchYear=="2021-22")
 {
     $YearSt="2021-04-01";
     $YearEnd="2022-03-31";
}

if ($searchYear=="2022-23")
 {
     $YearSt="2022-04-01";
     $YearEnd="2023-03-31";
}
...
<div class="container">
    ...
    width: 12%;" onChange="SearchAdvance(this.value, '<?php echo $searchSubject; ?>', '<?php echo $searchYear; ?>')">
    ...
    font-family: 'Lucida Sans', 'Lucida Sans Regular', 'Lucida Grande',
        'Lucida Sans Unicode', Geneva, Verdana, sans-serif;
    box-shadow: 2px 2px 2px 1px rgba(0, 0, 0, 0.2);
    width: 15%;" onChange="SearchAdvance('<?php echo $searchClass; ?>', this.value, '<?php echo $searchYear; ?>')">
			...
			<label for="Year" class="classDiv" style="padding: 10px;
        margin: 10px;font-weight: bold;
        font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
        margin: 1rem;
        margin-bottom: 2rem;
        font-size: 1.6rem;">Year: </label>
            <select name="searchYear" id="Year" class="subjectDiv" style="margin-left: 2.9rem;
    width: 12%;padding: 6px;
    width: 12%;
    margin: 1rem;font-size: 1.2rem;
    border-radius: 10px;
    font-family: 'Lucida Sans', 'Lucida Sans Regular', 'Lucida Grande',
        'Lucida Sans Unicode', Geneva, Verdana, sans-serif;
    box-shadow: 2px 2px 2px 1px rgba(0, 0, 0, 0.2);
    width: 12%;" onChange="SearchAdvance('<?php echo $searchClass; ?>', '<?php echo $searchSubject; ?>', this.value)">
             
			 <?php
			 if ($searchYear=="2022-23")
			 {
			   echo "<option value=\"2022-23\" Selected>2022-23</option>";
			 }
			 else
			 {
			 ...
			 if ($searchYear=="2021-22")
			 {
			   echo "<option value=\"2021-22\" Selected>2021-22</option>";
			 }
			 else
			 {
...
//SELECT DISTINCT(StResult.ID), StInfo.Status FROM StResult INNER JOIN StInfo ON StResult.ID=StInfo.ID WHERE StInfo.Status='Active'		
		...
		$sql1 = "SELECT DISTINCT(StResult.ID), StInfo.Status FROM StResult INNER JOIN StInfo ON StResult.ID=StInfo.ID WHERE StResult.ExamDate BETWEEN '$YearSt' AND '$YearEnd' ";
...
//$searchClass $searchSubject
        ...
        if ($searchYear=="2022-23")
        {
            $sql1 = $sql1 . " AND StInfo.Status='Active'";
        }
...
//$searchClass $searchSubject
        if ($result1->num_rows >= 0) 
		{
            while ($row = $result1->fetch_assoc()) 
			{
                ...
                $sqlAVG    = "SELECT Avg(Percentage), COUNT(ID) FROM StResult WHERE ID='$ID' AND ExamDate BETWEEN '$YearSt' AND '$YearEnd' ";
				...
				$sqlAVGMath    = "SELECT Avg(Percentage), COUNT(ID) FROM StResult WHERE ID='$ID' AND Subject='Math' AND ExamDate BETWEEN '$YearSt' AND '$YearEnd' ";
				...
				$sqlAVGScience    = "SELECT Avg(Percentage), COUNT(ID) FROM StResult WHERE ID='$ID' AND Subject='Science' AND ExamDate BETWEEN '$YearSt' AND '$YearEnd' ";
