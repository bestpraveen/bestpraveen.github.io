...
window.location.href = 'DipsMarksDetails_ReTest.php' + '?SearchAdvance=Yes' + '&searchClass=' + searchClass + '&searchSubject=' + searchSubject + '&searchExamTitle=' + searchExamTitle + '&searchName=' + searchName + "#Chart";
      }
...
if ($searchClass==15)
{
	$sqlR = "SELECT DISTINCT(ID) FROM StResult_ReTest ORDER BY Class ASC";
}
else
{
	$sqlR = "SELECT DISTINCT(ID) FROM StResult_ReTest WHERE Class='$searchClass' ORDER BY ID ASC";
}
...
if ($searchClass==15)
{
	$sqlL = "SELECT DISTINCT(Level) FROM StResult_ReTest ";
}
else
{
	$sqlL = "SELECT DISTINCT(Level) FROM StResult_ReTest WHERE Class='$searchClass'";
}
...
$sqlL= str_replace("StResult_ReTest AND", "StResult_ReTest WHERE ", $sqlL);
...
<form action="" method="get" id="formControler" style="text-align: center;
    ...
    width: 15%;" onChange="SearchAdvance('<?php echo $searchClass; ?>', '<?php echo $searchSubject; ?>', '<?php echo $searchExamTitle; ?>', this.value);">
            ...
            <!--<button type="submit" class="btn">Filter</button>-->
					...
					<?php if ($searchName!="All")
					{
						echo "<button type=\"submit\" class=\"btn\" style=\"color:blue;\"><a style=\"text-decoration:none;\" href=\"https://pa3tuitionpoint.000webhostapp.com/DipsMarksDetails_ReTest.php?SearchAdvance=Yes&searchClass=$searchClass&searchSubject=$searchSubject&searchExamTitle=$searchExamTitle&searchName=$searchName#Chart\">Graph</a></button>";
					}
					?>
...
$data1 = array();
        ...
        $sql1 = "SELECT * FROM StResult_ReTest WHERE ID='$id'";
		...
		$sql1 = "SELECT * FROM StResult_ReTest AND Class='$searchClass' AND Subject='$searchSubject' AND Level='$searchExamTitle' AND ID='$searchName'";

		//$sql1 = $sql1 . " ORDER BY Percentage DESC";
		$sql1 = $sql1 . " ORDER BY ExamDate, Percentage DESC";
		...
		$sql1= str_replace("StResult_ReTest AND", "StResult_ReTest WHERE ", $sql1);
		//echo "<br>2 sql1= $sql1";
       ...
       //echo "<br>Class=$searchClass, Subject= $Subject, title=$searchTitle, ID= $id,  Name=$searchName <br> sql1= $sql1 <br>";
        ...
        if ($result1->num_rows >= 0) 
		{
			...
			{                
				...
				if ($Subject=="Math") 
					{
					$ChartColor="rgba(0, 120, 0, 3)"; $ChartLebel="Math % (".$ID.")"; //green
					$jsonArrayItem = array();
				...
				if ($Subject=="Science") 
					{ 
					$ChartColor1="rgba(0, 0, 255, 1)"; $ChartLebel1="Science % (".$ID.")"; //blue
					$jsonArrayItem1 = array();
					$jsonArrayItem1['label'] = $ExamDate1;
					$jsonArrayItem1['value'] = $PercentageR;
					array_push($jsonArray1, $jsonArrayItem);
					array_push($label1, $ExamDate1);
					array_push($data1, $PercentageR);
					
					} 

				//if ($searchSubject=="All") { $ChartColor="rgba(50, 50, 150, 1)"; $ChartLebel="All Subject %";} //blue
...
$chartConfig1 = json_encode($chartConfigArr1);
//echo $chartConfig;


if ($searchName!="All" AND $searchSubject=="Math")
{
$chartUrl = 'https://quickchart.io/chart?w=330&h=200&c=' . urlencode($chartConfig);
echo " <p id='Chart'><img src=\"$chartUrl\"></p>";
}

if ($searchName!="All" AND $searchSubject=="Science")
{
$chartUrl1 = 'https://quickchart.io/chart?w=330&h=200&c=' . urlencode($chartConfig1);
echo " <p id='Chart1'><img src=\"$chartUrl1\"></p>";
}


if ($searchName!="All" AND $searchSubject=="All")
{
$chartUrl = 'https://quickchart.io/chart?w=330&h=200&c=' . urlencode($chartConfig);
echo " <p id='Chart'><img src=\"$chartUrl\"></p>";
